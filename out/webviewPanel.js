...
function createWebviewPanel(context, title) {
    const panel = vscode.window.createWebviewPanel('vdgGenerator', `VDG: ${title}`, vscode.ViewColumn.Beside, {
        enableScripts: true,
        localResourceRoots: [
    ...
    const cssUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(mediaPath, 'webview.css'));
    const jsUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(mediaPath, 'webview.js'));
    const d3Uri = panel.webview.asWebviewUri(vscode.Uri.joinPath(mediaPath, 'd3.v7.min.js'));
    panel.webview.html = getHtml(cssUri, jsUri, d3Uri);
    return panel;
}
function getHtml(cssUri, jsUri, d3Uri) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
