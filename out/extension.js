...
function activate(context) {
    const disposable = vscode.commands.registerCommand('vdg.generateChunks', async (uri) => {
        let projectPath;
        if (uri) {
        ...
        const panel = (0, webviewPanel_1.createWebviewPanel)(context, projectName);
        let disposed = false;
            ...
            if (message.type === 'start') {
                ...
                if (fs.existsSync(chunkTreePath)) {
                    try {
                        ...
                        (function stripSummaries(node) {
                            delete node.summary;
                            delete node.embedding;
                            if (node.children) {
                                node.children.forEach(stripSummaries);
                            }
                        })(tree);
                ...
                const logUri = await vscode.window.showSaveDialog({
                    ...
                    defaultUri: vscode.Uri.file(path.join(projectPath, 'log.txt')),
                });
                if (logUri) {
                ...
                const saveUri = await vscode.window.showSaveDialog({
                    ...
                    defaultUri: vscode.Uri.file(path.join(vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || '', `${projectName}-ast.png`)),
                });
                if (saveUri) {
...
function loadEdgesFromCsv(csvPath) {
    const edges = [];
...
function loadCachedData(panel, chunkTreePath, edgesPath, simEdgesPath, projectPath, isDisposed) {
    if (isDisposed()) {
        return;
    }
    try {
        ...
        (function checkSummaries(node) {
            if (node.summary) {
                hasSummaries = true;
            }
            if (!hasSummaries && node.children) {
                node.children.forEach(checkSummaries);
            }
        })(treeData);
...
function runAnalysis(context, panel, projectPath, maxNodeSize, preserveScoping, isDisposed) {
...
function runSummarization(context, panel, chunkTreePath, isDisposed) {
    const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
...
function runSimilarityRecompute(context, panel, chunkTreePath, topPercent, simEdgesPath, projectPath, edgesPath, isDisposed) {
    const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
...
function checkNeo4jStatus(panel) {
...
function runNeo4jLoader(context, panel, action, vdgDir, isDisposed) {
    const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
...
function runInvestigation(context, panel, chunkTreePath, nodeId, isDisposed) {
    const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
        ...
        for (const line of lines) {
            ...
            try {
                ...
                if (msg.type === 'log') {
                    ...
                    try {
                        const tree = JSON.parse(fs.readFileSync(chunkTreePath, 'utf-8'));
                        (function saveInvestigation(node) {
                            if (node.id === nodeId) {
